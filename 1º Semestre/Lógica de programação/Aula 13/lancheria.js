...
function listagem() {
    titulo("Lista dos Itens do Cardápio")

    console.log("Nº Nome...............: Categoria: Ingredientes...........................: Preço R$")
    console.log("------------------------------------------------------------------------------------")
    
    for (let i = 0; i < lanches.length; i++) {
        console.log(`${String(i+1 ).padStart(2)} ${lanches[i].padEnd(20)} ${categorias[i].padEnd(10)} ${ingredientes[i].padEnd(40)} ${precos[i].toFixed(2).padStart(8)}`)
    }
...
function pesquisaCategoria() {
    ...
    console.log("Nome...............: Ingredientes...........................: Preço R$")
    console.log("----------------------------------------------------------------------")
    ...
    for (let i = 0; i < lanches.length; i++) {
        if (categorias[i].toUpperCase() == categoria){
            console.log(`${lanches[i].padEnd(20)} ${ingredientes[i].padEnd(40)} ${precos[i].toFixed(2).padStart(8)}`)
            existe = existe + 1
        }
    }
...
function exclusao() {
    ...
    console.log()
    const num = Number(prompt("Qual Nº do Produto Excluir (0, para cancelar)? "))

    if (num == 0 || num > lanches.length){
        console.log("Nenhum produto excluído...")
        return
    }

    //exclui os elementos do vetor
    lanches.slice(num-1, 1)
    categorias.splice(num-1, 1)
    ingredientes.slice(num-1, 1)
    precos.slice(num-1, 1)
    fotos.slice(num-1, 1)

    console.log("Ok. Produto removido com sucesso")
}

// Função para salvar dados em um arquico
function gravaProdutos() {
    ...
    console.log("Ok! Lista de produtos salvos com sucesso")
}
...
<body>
    ...
    console.log("Cardápio gerado com sucesso. Acesse: file:///C:/Users/Reny/Desktop/Faculdade/L%C3%B3gica%20de%20programa%C3%A7%C3%A3o/Aula%2013/cardapio.html")
}
